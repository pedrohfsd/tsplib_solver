...
class TSP_MIP_MTZ {
	...
	private:
		void addDecisionVariables(IloModel model, IloNumVarArray var, IloRangeArray con, Data& data) {
		...
		void addObjectiveFunction(IloModel model, IloNumVarArray var, IloRangeArray con, Data& data) {
			IloEnv env = model.getEnv();
			IloExpr expr(env);
			for (int i = 0; i < data.n; i++) {
				for (int j = 0; j < data.n; j++) {
		...
		void addDegreeConstraints(IloModel model, IloNumVarArray var, IloRangeArray con, Data& data) {
			IloEnv env = model.getEnv();
			for (int i = 0; i < data.n; i++) {
				IloExpr expr(env);
				for (int j = 0; j < data.n; j++) 
		...
		void addSubtourEliminationConstraint(IloModel model, IloNumVarArray var, IloRangeArray con, Data& data)	{
			IloEnv env = model.getEnv();
		...
		void print(IloCplex cplex, IloNumVarArray var, IloRangeArray con) {
